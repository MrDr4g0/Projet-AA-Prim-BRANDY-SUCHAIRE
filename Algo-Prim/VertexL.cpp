...
VertexL::VertexL(unsigned int number, const OurList<edgeL>& initialNeighbors) : v_number(number), v_neighbors(initialNeighbors)
{
}

void VertexL::AddNeighbor(unsigned int value, unsigned int cost)
{
	edgeL newNeighbor{ cost, value };
	v_neighbors.insert_back(newNeighbor);
}

OurList<edgeL> VertexL::getNeighbors()
{
	return v_neighbors;
}
